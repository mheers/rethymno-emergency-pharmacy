...
// refreshRetryDelay is the backoff after a failed daily refresh. The
// schedule image is normally fetched exactly once per day, at local
// midnight; a failing upstream retries hourly until it succeeds so the
// cache never goes permanently stale.
const refreshRetryDelay = time.Hour
...
// next refresh is scheduled for the following local midnight; on failure it
// is retried after refreshRetryDelay while the last known result keeps
// being served.
func (s *Server) refreshNow(ctx context.Context) {
	...
	if err != nil {
		s.log.Printf("schedule refresh failed: %v (retry in %s)", err, refreshRetryDelay)
		s.nextRefresh = s.now().Add(refreshRetryDelay)
		return
	}
	if e != nil {
